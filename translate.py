...
def translate_to_ukraine(scroll_root, root, image_listbox, data, out_list, name_entry, load_canvas, out_dim_entry,
                         h_dim_entry, alpha_entry, num_epochs_entry, batch_size_entry):
    destroy_all(deleted=widgets_list)
    files_button = Button(scroll_root, command=lambda: open_files(image_listbox, data['Language']),
                          text="Виберіть файли")
    files_button.pack()
